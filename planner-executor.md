...
*   **Sections and their primary purpose:**
    ...
    *   `High-level Task Breakdown`: The Planner outlines the major tasks or features of the project in this section of `.cursor/scratchpad.md`. For each major task listed, it should primarily specify the dedicated task file (e.g., `.cursor/feature-x-tasks.md`) where the detailed breakdown, sub-tasks, success criteria, and progress tracking are managed. This section itself should *not* contain granular sub-task lists or checkboxes; these details belong in the respective dedicated task files. The main purpose is to provide a high-level overview of the project's structure and point to where details for each major component can be found.
